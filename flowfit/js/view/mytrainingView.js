        ...
        main.innerHTML = `
            ...
            <section class="mytraining-content">
                ${trainings.map((training, index) => `
                    <div class="training-block ${index % 2 === 0 ? 'left' : 'right'}">
                        <div class="training-image">
                            <img src="${training.image}" alt="${training.title}">
                        </div>
                        <div class="training-details">
                            <h2>${training.title}</h2>
                            <p>Tempo de Execução: <strong>${training.time}</strong></p>
                            <p>Peso Recomendado: <strong>${training.weight}</strong></p>
                            <p>Tempo de Descanso: <strong>${training.rest}</strong></p>
                        </div>
                    </div>
                    <div class="training-description">
                        ${training.description.map(item => `<p class="list${index % 2 === 0 ? 'left' : 'right'}">${item}</p>`).join('')}
                    </div>
                `).join('')}
            </section>
